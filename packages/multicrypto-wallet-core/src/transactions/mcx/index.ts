...
export class MCXTxProvider {
  ...
  getHash(params: { tx: string }) {
    const multicoreTx = new this.lib.Transaction(params.tx);
    return multicoreTx.hash;
  }
  ...
  sign(params: { tx: string; keys: Array<Key>; utxos: any[]; pubkeys?: any[]; threshold?: number; opts: any }) {
    ...
    let multicoreTx = new this.lib.Transaction(tx);
    let applicableUtxos = this.getRelatedUtxos({
      outputs: multicoreTx.inputs,
      utxos
    });
    multicoreTx.associateInputs(applicableUtxos, pubkeys, threshold, opts);
    const privKeys = _.uniq(keys.map(key => key.privKey.toString()));
    const signedTx = multicoreTx.sign(privKeys).toString();
    return signedTx;
  }
