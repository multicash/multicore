...
describe('Ripple Api', function() {
  ...
  it('should transform a ripple rpc response into a multicore transaction', async () => {
    ...
    for (const tx of txs) {
      ...
      if (tx.outcome.deliveredAmount) {
        ...
        expect(Number(tx.outcome.balanceChanges[multicoreTx.to!][0].value)).to.be.gt(0);
      }
    }
  });
  ...
  it('should tag txs from a wallet', async () => {
    ...
    for (const tx of txs) {
      const multicoreTx = (await XRP.transform(tx, network)) as IXrpTransaction;
      const multicoreCoins = XRP.transformToCoins(tx, network);
      const { transaction, coins } = await XRP.tag(chain, network, multicoreTx, multicoreCoins);
  ...
  it('should save tagged transactions to the database', async () => {
    ...
    for (const tx of txs) {
      const multicoreTx = XRP.transform(tx, network) as IXrpTransaction;
      const multicoreCoins = XRP.transformToCoins(tx, network);
      const { transaction, coins } = await XRP.tag(chain, network, multicoreTx, multicoreCoins);
