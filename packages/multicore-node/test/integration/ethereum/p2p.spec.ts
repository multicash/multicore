import * as multicoreClient from 'multicore-client';
...
const { StreamUtil } = multicoreClient;
const chain = 'ETH';
...
async function getWallet() {
  let wallet: multicoreClient.Wallet;
  try {
    wallet = await multicoreClient.Wallet.loadWallet({ name });
    await wallet.register();
    await wallet.syncAddresses();
    return wallet;
  } catch (e) {
    console.log('Creating a new ethereum wallet');
    wallet = await multicoreClient.Wallet.create({
      name,
      chain,
      network,
      baseUrl,
