...
function findConfig(): ConfigType | undefined {
  ...
  const configFileName = 'multicore.config.json';
  let multicoreConfigPaths = [
  ...
  if (overrideConfig) {
    multicoreConfigPaths.unshift(overrideConfig);
  }
  // No config specified. Search home, multicore and cur directory
  for (let path of multicoreConfigPaths) {
    if (!foundConfig) {
      try {
        const expanded = path[0] === '~' ? path.replace('~', homedir()) : path;
        const multicoreConfig = require(expanded) as { multicoreNode: ConfigType };
        foundConfig = multicoreConfig.multicoreNode;
      } catch (e) {
...
const Config = function(): ConfigType {
  let config: ConfigType = {
    ...
    dbName: process.env.DB_NAME || 'multicore',
