          ...
          .pipe(
            new Transform({
              ...
              transform: async (data, _, cb) => {
                ...
                for (const tx of txs) {
                  const multicoreTx = this.provider.transform(tx, network) as IXrpTransaction;
                  const multicoreCoins = this.provider.transformToCoins(tx, network);
                  const { transaction, coins } = await this.provider.tag(chain, network, multicoreTx, multicoreCoins);
