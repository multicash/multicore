...
export class MultiCashP2PWorker extends BaseP2PWorker<IBtcBlock> {
  protected multicoreLib: any;
  protected multicoreP2p: any;
  ...
  constructor({ chain, network, chainConfig, blockModel = BitcoinBlockStorage }) {
    ...
    this.multicoreLib = Libs.get(chain).lib;
    this.multicoreP2p = Libs.get(chain).p2p;
    ...
    this.invCache = {};
    this.invCacheLimits = {
      [this.multicoreP2p.Inventory.TYPE.BLOCK]: 100,
      [this.multicoreP2p.Inventory.TYPE.TX]: 100000
    };
    this.messages = new this.multicoreP2p.Messages({
      network: this.multicoreLib.Networks.get(this.network)
    });
    this.pool = new this.multicoreP2p.Pool({
  ...
  setupListeners() {
    ...
    this.pool.on('peertx', async (peer, message) => {
      ...
      if (this.isSyncingNode && !this.isCachedInv(this.multicoreP2p.Inventory.TYPE.TX, hash)) {
        this.cacheInv(this.multicoreP2p.Inventory.TYPE.TX, hash);
    ...
    this.pool.on('peerblock', async (peer, message) => {
      ...
      const blockInCache = this.isCachedInv(this.multicoreP2p.Inventory.TYPE.BLOCK, hash);
      if (!blockInCache) {
        block.transactions.forEach(transaction => this.cacheInv(this.multicoreP2p.Inventory.TYPE.TX, transaction.hash));
        this.cacheInv(this.multicoreP2p.Inventory.TYPE.BLOCK, hash);
      }
    ...
    this.pool.on('peerinv', (peer, message) => {
      if (this.isSyncingNode) {
        const filtered = message.inventory.filter(inv => {
          const hash = this.multicoreLib.encoding
