...
export class Utils {
  ...
  static deriveAddress(scriptType, publicKeyRing, path, m, network, coin) {
    ...
    var multicore = Bitcore_[chain];
    var publicKeys = _.map(publicKeyRing, item => {
      var xpub = new multicore.HDPublicKey(item.xPubKey);
      return xpub.deriveChild(path).publicKey;
    });

    var multicoreAddress;
    switch (scriptType) {
      ...
      case Constants.SCRIPT_TYPES.P2PKH:
        ...
        if (Constants.UTXO_COINS.includes(coin)) {
          ...
          multicoreAddress = Deriver.deriveAddress(
            chain.toUpperCase(),
            network,
            xPubKey,
            addressIndex,
    ...
    return {
      address: multicoreAddress.toString(true),
      path,
  ...
  static buildTx(txp) {
    ...
    if (Constants.UTXO_COINS.includes(coin)) {
      var multicore = Bitcore_[coin];

      var t = new multicore.Transaction();
      ...
      if (txp.toAddress && txp.amount && !txp.outputs) {
        ...
        _.each(txp.outputs, o => {
          ...
          if (o.script) {
            t.addOutput(
              new multicore.Transaction.Output({
                script: o.script,
                satoshis: o.amount
              })
            );
