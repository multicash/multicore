...
export class API extends EventEmitter {
  ...
  static BitcoreMcx = CWC.BitcoreLibMcx;
  ...
  _addSignaturesToBitcoreTxBitcoin(txp, t, signatures, xpub) {
    ...
    const multicore = Bitcore_[txp.coin];
    if (signatures.length != txp.inputs.length)
      throw new Error('Number of signatures does not match number of inputs');

    let i = 0;
    const x = new multicore.HDPublicKey(xpub);

    _.each(signatures, signatureHex => {
      try {
        const signature = multicore.crypto.Signature.fromString(signatureHex);
        ...
        const s = {
          ...
          sigtype:
            ...
            multicore.crypto.Signature.SIGHASH_ALL |
            multicore.crypto.Signature.SIGHASH_FORKID,
          publicKey: pub
        };
