...
export class BtcChain implements IChain {
  ...
  constructor(private multicoreLib = BitcoreLib) {
  ...
  getDustAmountValue() {
    return this.multicoreLib.Transaction.DUST_AMOUNT;
  }
  ...
  checkDust(output) {
    const dustThreshold = Math.max(Defaults.MIN_OUTPUT_AMOUNT, this.multicoreLib.Transaction.DUST_AMOUNT);
  ...
  getEstimatedSizeForSingleOutput(address?: string) {
    let addressType = '';
    if (address) {
      const a = this.multicoreLib.Address(address);
      addressType = a.type;
    }
  ...
  getBitcoreTx(txp, opts = { signed: true }) {
    const t = new this.multicoreLib.Transaction();
    ...
    _.each(txp.outputs, o => {
      ...
      if (o.script) {
        t.addOutput(
          new this.multicoreLib.Transaction.Output({
            script: o.script,
            satoshis: o.amount
          })
        );
  ...
  checkTx(txp) {
    ...
    try {
      const multicoreTx = this.getBitcoreTx(txp);
      multicoreError = multicoreTx.getSerializationError(serializationOpts);
      if (!multicoreError) {
        txp.fee = multicoreTx.getFee();
      }
    } catch (ex) {
    ...
    if (multicoreError instanceof this.multicoreLib.errors.Transaction.FeeError)
    ...
    if (multicoreError instanceof this.multicoreLib.errors.Transaction.DustOutputs) return Errors.DUST_AMOUNT;
    return multicoreError;
  }
  ...
  addSignaturesToBitcoreTx(tx, inputs, inputPaths, signatures, xpub, signingMethod) {
    ...
    let i = 0;
    const x = new this.multicoreLib.HDPublicKey(xpub);

    _.each(signatures, signatureHex => {
      try {
        const signature = this.multicoreLib.crypto.Signature.fromString(signatureHex);
        const pub = x.deriveChild(inputPaths[i]).publicKey;
        const s = {
          inputIndex: i,
          signature,
          sigtype: this.multicoreLib.crypto.Signature.SIGHASH_ALL | this.multicoreLib.crypto.Signature.SIGHASH_FORKID,
          publicKey: pub
        };
  ...
  validateAddress(wallet, inaddr, opts) {
    const A = this.multicoreLib.Address;
    let addr: {
      network?: string;
