...
const Bitcore = {
  ...
  eth: require('multicore-lib'),
  xrp: require('multicore-lib'),
...
export class Wallet {
  ...
  updateBEKeys() {
    ...
    var seed =
      ...
      this.coin +
      salt;
    seed = multicore.crypto.Hash.sha256(Buffer.from(seed));
    const priv = multicore.PrivateKey(seed, this.network);
