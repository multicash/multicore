...
const  CWC = require('multicrypto-wallet-core');
...
var Bitcore = require('multicore-lib');
var Bitcore_ = {
  btc: Bitcore,
  ...
  _.each(testSet, function(x) {
    ...
    describe('#createTx ' + coin + ' flags' + JSON.stringify(flags), function() {
      ...
      describe('Tx proposal creation & publishing ' + coin, function() {
        ...
        it('should fail gracefully when multicore throws exception on raw tx creation', function(done) {
          helpers.stubUtxos(server, wallet, 1, { coin }, function() {
            ...
            var multicoreStub;
            var multicoreStub = sandbox.stub(CWC.BitcoreLib, 'Transaction');
            multicoreStub.throws({
              name: 'dummy',
              message: 'dummy exception'
            });
            var multicoreStub = sandbox.stub(CWC.BitcoreLibCash, 'Transaction');
            multicoreStub.throws({
              name: 'dummy',
              message: 'dummy exception'
            });
            ...
            server.createTx(txOpts, function(err, tx) {
              ...
              if(multicoreStub) multicoreStub.restore();
              cwcStub.restore();
              done();
            });
          });
          ...
          it('should create tx with 0 change output', function(done) {
            helpers.stubUtxos(server, wallet, 1, function() {
              ...
              server.createTx(txOpts, function(err, tx) {
                should.not.exist(err);
                should.exist(tx);
                var multicoreTx = ChainService.getBitcoreTx(tx);
                multicoreTx.outputs.length.should.equal(1);
                multicoreTx.outputs[0].satoshis.should.equal(tx.amount);
                done();
              });
            });
          });
