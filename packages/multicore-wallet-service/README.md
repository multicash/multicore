...
[![NPM Package](https://img.shields.io/npm/v/multicore-wallet-service.svg?style=flat-square)](https://www.npmjs.org/package/multicore-wallet-service)
[![Build Status](https://img.shields.io/travis/bitpay/multicore-wallet-service.svg?branch=master&style=flat-square)](https://travis-ci.org/bitpay/multicore-wallet-service)
[![Coverage Status](https://coveralls.io/repos/bitpay/multicore-wallet-service/badge.svg?branch=master)](https://coveralls.io/r/bitpay/multicore-wallet-service?branch=master)
...
See [multicore-wallet-client](https://github.com/bitpay/multicore/tree/master/packages/multicore-wallet-client) for the _official_ client library that communicates to BWS and verifies its response. Also check [multicore-wallet](https://github.com/bitpay/multicore/tree/master/packages/multicore-wallet) for a simple CLI wallet implementation that relies on BWS.
...
More about BWS at https://blog.bitpay.com/announcing-the-multicore-wallet-suite/
...
BWS supports SSL and Clustering. For a detailed guide on installing BWS with extra features see [Installing BWS](https://github.com/bitpay/multicore/blob/master/packages/multicore-wallet-service/installation.md).
...
- A copayer could join the wallet more than once, and there is no mechanism to prevent this. See [wallet](https://github.com/bitpay/multicore/tree/master/packages/multicore-wallet)'s confirm command, for a method for confirming copayers.
...
@dabura667 made a report about how to use letsencrypt with BWS: https://github.com/bitpay/multicore-wallet-service/issues/423
...
- multicore-node running on http://localhost:3000
...
> **multicore-wallet-service/lib/notificationbroadcaster.js**
> Note: If testing on a PC browser, comment out notificationbroadcaster.js to disable notifications.

### Steps:

**multicore.config.json**

1.  Add regtest to multicore.config.json.
...
**multicore-wallet-service/config.js**
...
See [Bitcore Wallet Client](https://github.com/bitpay/multicore/tree/master/packages/multicore-wallet-client) for implementation details.
...
- Wallet object. (see [fields on the source code](https://github.com/bitpay/multicore/blob/master/packages/multicore-wallet-service/src/lib/model/wallet.ts)).
...
- List of pending TX Proposals. (see [fields on the source code](https://github.com/bitpay/multicore/blob/master/packages/multicore-wallet-service/src/lib/model/txproposal.ts))
...
- List of Addresses object: (https://github.com/bitpay/multicore/blob/master/packages/multicore-wallet-service/src/lib/model/address.ts). This call is mainly provided so the client check this addresses for incoming transactions (using a service like [Insight](https://insight.multicore.io)
...
- TX Proposal object. (see [fields on the source code]https://github.com/bitpay/multicore/blob/master/packages/multicore-wallet-service/src/lib/model/txproposal.ts)). `.id` is probably needed in this case.

### `/v2/txproposals/:id/publish`: Publish the previously created `temporary` tx proposal

Returns:

- TX Proposal object. (see [fields on the source code](https://github.com/bitpay/multicore/blob/master/packages/multicore-wallet-service/src/lib/model/txproposal.ts)).

### `/v3/addresses/`: Request a new main address from wallet . (creates an address on normal conditions)

Returns:

- Address object: (https://github.com/bitpay/multicore/blob/master/packages/multicore-wallet-service/src/lib/model/address.ts). Note that `path` is returned so client can derive the address independently and check server's response.
...
- TX Proposal object. (see [fields on the source code](https://github.com/bitpay/multicore/blob/master/packages/multicore-wallet-service/src/lib/model/txproposal.ts)). `.status` is probably needed in this case.
