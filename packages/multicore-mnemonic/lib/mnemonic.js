...
const BN = multicore.crypto.BN;
const unorm = require('unorm');
const _ = multicore.deps._;

const pbkdf2 = require('./pbkdf2');
const errors = require('./errors');

const Hash = multicore.crypto.Hash;
const Random = multicore.crypto.Random;

const $ = multicore.util.preconditions;
...
var Mnemonic = function(data, wordlist) {
  ...
  if (Buffer.isBuffer(data)) {
    ...
    throw new multicore.errors.InvalidArgument('data', 'Must be a Buffer, a string or an integer');
  }
  ent = ent || 128;
  ...
  if (ent % 32 !== 0 || ent < 128 || ent > 256) {
    throw new multicore.errors.InvalidArgument('ENT', 'Values must be ENT > 128 and ENT < 256 and ENT % 32 == 0');
  }
...
Mnemonic.prototype.toHDPrivateKey = function(passphrase, network) {
  ...
  return multicore.HDPrivateKey.fromSeed(seed, network);
};
