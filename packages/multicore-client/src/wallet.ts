...
import { Deriver, Transactions } from 'multicrypto-wallet-core';
import 'source-map-support/register';
import { Client } from './client';
import { Encryption } from './encryption';
import { Storage } from './storage';
const { PrivateKey } = require('multicrypto-wallet-core').BitcoreLib;
const Mnemonic = require('multicore-mnemonic');
...
export class Wallet {
  ...
  static async create(params: Partial<WalletObj>) {
    ...
    // save wallet to storage and then multicore-node
    ...
    await loadedWallet.register().catch(e => {
      console.debug(e);
      console.error('Failed to register wallet with multicore-node.');
    });
