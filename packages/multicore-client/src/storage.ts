...
const multicoreLib = require('multicrypto-wallet-core').BitcoreLib;

export class Storage {
  path: string;
  db: Array<Mongo | Level>;
  collection: 'multicoreWallets';
  url?: string;
  ...
  async addKeys(params: { name: string; keys: KeyImport[]; encryptionKey: string }) {
    ...
    for (const key of keys) {
      let { pubKey } = key;
      pubKey = pubKey || new multicoreLib.PrivateKey(key.privKey).publicKey.toString();
      let payload = {};
